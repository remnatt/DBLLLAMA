...
namespace LLAMA
{
    ...
    // You'll need to implement this helper class
    public static class SerializationHelper
    {
        public static byte[] SerializeLongList(List<long> list)
        {
            var result = new List<byte>();
            result.AddRange(BitConverter.GetBytes(list?.Count ?? 0));
            if (list != null)
            {
        ...
        public static (List<long>, int) UnserializeLongList(byte[] data)
        {
            ...
            {
                list.Add(BitConverter.ToInt64(data, offset));
                offset += 8;
            }
        ...
        public static byte[] SerializeVLong(long value)
        {
            return BitConverter.GetBytes(value);
        }

        public static (long, int) UnserializeVLong(byte[] data)
        {
            if (data.Length >= 8)
            {
                return (BitConverter.ToInt64(data, 0), 8);
            }
            return (0, 0);
        }
    }
